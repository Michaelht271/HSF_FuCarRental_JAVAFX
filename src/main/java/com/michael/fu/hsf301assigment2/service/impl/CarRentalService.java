...
import com.michael.fu.hsf301assigment2.repository.CustomerRepository;
...
public class CarRentalService extends BaseService {
    ...
    private void validateCarAvailability(Car car, LocalDate pickupDate) {
        ...
        public long countRentalsBetween(LocalDate start, LocalDate end) {
            return carRentalRepository.countByRentDateBetween(start, end);
        }


        public BigDecimal sumRevenueBetween(LocalDate start, LocalDate end) {
            BigDecimal sum = carRentalRepository.sumRentPriceBetween(start, end);
